"""
Super-Mario-Bros2-v1 -- Double Deep Q-learning
"""
...
class Agent:
    ...
    def _build_model(self):
        ...
        model.add(Conv2D(32, (8, 8), strides=(4, 4),
                  activation='relu', input_shape=self.observation_shape))
...
if __name__ == "__main__":
    ...
    # Loads the weights
    if os.path.isfile("super_mario_bros2_v1_dqqn.h5"):
        agent.load_weights("super_mario_bros2_v1_ddqn.h5")
    ...
    for episode in range(num_episodes):
        ...
        if total_reward > max_reward:
            agent.save_weights("super_mario_bros2_v1_ddqn.h5")
    ...
    with open("dqnn_avg_rewards.csv", "w") as f:
        for reward in avg_rewards:
            f.write(f"{reward}\n")

    with open("dqnn_avg_game_scores.csv", "w") as f:
        for game_score in avg_game_scores:
            f.write(f"{game_score}\n")

    with open("dqnn_avg_steps_per_episode.csv", "w") as f:
        for steps in avg_steps_per_episode:
            f.write(f"{steps}\n")

    with open("dqnn_training_times.csv", "w") as f:
        for time in training_times:
            f.write(f"{time}\n")

    # Print average metrics over all episodes
    with open("dqnn_avg_metrics.csv", "w") as f:
        # column headers
        f.write(
