"""
Super-Mario-Bros2-v1 -- MobileNet
"""
...
class Agent:
    ...
    def _build_model(self):
        model = Sequential()
        model.add(MobileNet(weights=None, include_top=False,
                  input_shape=self.observation_shape))
        model.add(Flatten())
        model.add(Dense(512, activation='elu',
                  kernel_initializer='random_uniform'))
        model.add(Dense(self.action_size, activation='softmax', name='output'))
    ...
    def experience_reply(self):
        ...
        indices = np.random.choice(
            range(len(self.memory)), size=self.batch_size)
        ...
        next_state_sample = np.array(
            [self.memory.next_state[i][0] for i in indices])
...
if __name__ == "__main__":
    ...
    env = wrap_nes("SuperMarioBros2-v1", actions.SIMPLE_MOVEMENT)

    # Records the environment
    if monitor:
        env = gym.wrappers.Monitor(
            env, "recording", video_callable=lambda episode_id: True, force=True)
    ...
    # Loads the weights
    if os.path.isfile("super_mario_bros2_v1_mobile.h5"):
        agent.load_weights("super_mario_bros2_v1_mobile.h5")
    ...
    for episode in range(num_episodes):
        ...
        for episode_step in range(num_episode_steps):
            ...
            next_state = np.reshape(
                observation, (1,) + env.observation_space.shape)
        ...
        if total_reward > max_reward:
            agent.save_weights("super_mario_bros2_v1_mobile.h5")
    ...
    with open("mobile_avg_rewards.csv", "w") as f:
        for reward in avg_rewards:
            f.write(f"{reward}\n")

    with open("mobile_avg_game_scores.csv", "w") as f:
        for game_score in avg_game_scores:
            f.write(f"{game_score}\n")

    with open("mobile_avg_steps_per_episode.csv", "w") as f:
        for steps in avg_steps_per_episode:
            f.write(f"{steps}\n")

    with open("mobile_training_times.csv", "w") as f:
        for time in training_times:
            f.write(f"{time}\n")

    # Print average metrics over all episodes
    with open("mobile_avg_metrics.csv", "w") as f:
        # column headers
        f.write(
            "Average Reward, Average Game Score, Average Steps per Episode, Training Time\n")
        # data
        f.write(
            f"{avg_reward_overall}, {avg_game_score_overall}, {avg_steps_overall}, {avg_training_time}\n")

    print(f"Average Reward over {num_episodes} episodes: {avg_reward_overall}")
    print(
        f"Average Game Score over {num_episodes} episodes: {avg_game_score_overall}")
    print(
        f"Average Steps per Episode over {num_episodes} episodes: {avg_steps_overall}")
